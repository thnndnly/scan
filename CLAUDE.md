...
ui.py                 # Gradio web interface (8 tabs, all labels in German)
cli.py                # Click CLI: scan, ui, dataset, db, catalog, archive, label, config
detection/
  opencv_detector.py  # Canny edges → IoU-NMS → perspective-correct patches
  yolo_detector.py    # YOLOv8 (optional, needs [yolo] extra)
recognition/
  ocr_recognizer.py   # EasyOCR on top-13% title strip + rapidfuzz match
  hash_recognizer.py  # pHash of artwork region vs. SQLite DB
...
utils/
  image_utils.py      # load_image() — OpenCV with Pillow fallback for AVIF/HEIC
  fuzzy_search.py     # rapidfuzz wrapper against data/card_names.json
...
The OpenCV detector uses **non-maximum suppression** (IoU threshold 0.3) to
remove duplicate detections. RETR_LIST finds all contours including inner card
frames; NMS keeps only the largest non-overlapping box per card.
...
detection:
  ...
  min_card_area_px: 2000
  save_debug: false         # Save edge map + detection image to output/debug/

recognition:
  primary_method: ocr       # 'ocr' or 'hash'
  fallback_method: hash     # 'ocr', 'hash', or 'llm'
...
- **Hash DB**: not included in repo (requires large card image downloads). OCR works without it.
- **Card catalog**: not included in repo (~80 MB SQLite). Build with `mtg-scan catalog build`.
- **Scryfall rate limit**: 110 ms between requests — do not parallelize lookups.
...
- **Dataset logger**: enabled by default. Set `dataset.enabled: false` in `config.yaml` to disable. Pipeline continues normally if the DB cannot be opened.
...
| Tab | Purpose |
|-----|---------|
| Scanner | Upload image, run scan, patch gallery with card names + prices |
| Scan-Historie | Browse past scans, click row to view patches |
| Dataset Explorer | Review low-confidence patches, search Scryfall, save corrections |
| Hash-DB | Build/status the perceptual hash DB with dry-run preview |
| Karten-Zuordnung | Assign patches to specific catalog entries (set/printing/finish) |
